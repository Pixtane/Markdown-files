...
> Homework for 07.09.2022
 ...
 1. **double ldexp(double x, int exponent)** - множить число на 2 в заданому ступені. `double ldexp(0.65, 3)` = 0.65 * 2^3^ = 0.65 * 8 = **5.2**
 2. **double pow(double x, double y)** повертає **x** піднятий до ступеня **y** тобто x^y^.  Наприклад якщо `double x = 5` та `double y = 2` то `pow(double x, double y)` буде *25*  
